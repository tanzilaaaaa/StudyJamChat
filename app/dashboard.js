...
import { addNotification, getUnreadNotificationCount, loadCourses, loadGroups, saveCourses, saveGroups } from "../src/storage";
...
export default function Dashboard() {
  ...
  const createWelcomeNotification = async (user) => {
    ...
    const welcomeShownKey = `@welcome_shown_${user.uid}`;
    const welcomeShown = await AsyncStorage.getItem(welcomeShownKey);
    
    if (!welcomeShown) {
      // First time user - create welcome notification
      await addNotification(user.uid, {
        type: 'system',
        title: 'Welcome to StudyJam! 🎉',
        message: `Hi ${user.displayName || user.email}! Start by joining a study group or creating a new course.`,
      });
